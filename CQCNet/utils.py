...
# 示例调用
# save_imgs(img, msk, msk_pred, 0, './path_to_save/', 'dataset_name')
...
class CELoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        return self.celoss(pred_, target_)
...
class BceDiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        loss = self.wd * diceloss + self.wb * 0.5 * bceloss
        return loss


class BceDiceFocal(nn.Module):
    def __init__(self):
        super(BceDiceFocal, self).__init__()
        # 初始化可学习参数 sigma1, sigma2 和 sigma3
    ...
    def forward(self, pred, target):
        ...
        combined_loss = bce_loss + dice_loss + focal_loss
...
class UncertaintyBceDice(nn.Module):
    def __init__(self):
        super(UncertaintyBceDice, self).__init__()
        # 初始化可学习参数 sigma1 和 sigma2
        self.log_sigma1 = nn.Parameter(torch.zeros(1, requires_grad=True))
        self.log_sigma2 = nn.Parameter(torch.zeros(1, requires_grad=True))
    ...
    def forward(self, pred, target):
        ...
        combined_loss = (
            (1 / (sigma1**2)) * bce_loss +
            (1 / (sigma2**2)) * dice_loss +
            2 * self.log_sigma1 + 2 * self.log_sigma2
        )

        return combined_loss


class UncertaintyBceDiceFocal(nn.Module):
    def __init__(self):
        super(UncertaintyBceDiceFocal, self).__init__()
        # 初始化可学习参数 sigma1, sigma2 和 sigma3
        self.log_sigma1 = nn.Parameter(torch.zeros(1, requires_grad=True))
        self.log_sigma2 = nn.Parameter(torch.zeros(1, requires_grad=True))
        self.log_sigma3 = nn.Parameter(torch.zeros(1, requires_grad=True))
    ...
    def forward(self, pred, target):
        ...
        # Learnable weights (uncertainty parameters)
        sigma1 = torch.exp(self.log_sigma1)
        sigma2 = torch.exp(self.log_sigma2)
    ...
    def __init__(self, data_name, train=True):
        if data_name == 'isic18':
            ...
            if train:
                ...
                self.mean = 162.834
                self.std = 48.181
        elif data_name == 'cervix_acid_split':
            if train:
                self.mean = 162.322
                self.std = 47.825
            else:
